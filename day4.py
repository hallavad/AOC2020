...
def validate_cred_val(cred):
    ...
    val_f = {
            ...
            'hgt': lambda v: (v[-2:] == 'in' and 59 <= int(v[:-2]) <= 76) or
                             (v[-2:] == 'cm' and 150 <= int(v[:-2]) <= 193),
            'hcl': lambda v: v[0] == '#' and all([c in hex_c for c in v[1:]]),
            'ecl': lambda v: v in valid_ecl,
            'pid': lambda v: len(v) == 9 and all([c in num for c in v]),
    }
    valid_fields = {'byr': False, 'iyr': False, 'eyr': False, 'hgt': False,
